...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            if message.chat.id == SUPPORT_CHAT_ID:
                await message.reply_text(f"""<b>Hi  {message.from_user.mention},

မင်းရှာတဲ့ {search} ကားဒီမှာ မရှိဘူး ဟေရောင်

ငါ့ဟိုဖက်ဂရုမှာ {str(total_results)}  ခု ရှာတွေထားတယ်။

ဟိုဖက်ဂရု ကို လစ်ကွာ 🤪🤪

👇👇👇👇

 . \n\nFᴏʀ Mᴏᴠɪᴇs, Jᴏɪɴ @MKS_REQUESTGROUP </b>""")
            else:
